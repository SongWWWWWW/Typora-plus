...
- `IIndexService`: asynchronous workspace indexing, cross-file search, Markdown metadata collection, and backlink/link graph queries
...
- SQLite-backed index provider for `IIndexService`: persisted search, metadata, and link graph queries
