...
Export is a provider-backed platform service. `IExportService` owns provider registration and save routing; the Markdown package contributes the current HTML provider using `marked`, while the Electron main process owns the save dialog and bounded file write. When export providers need note resources, `IExportService` injects a resolver backed by `IResourceService` into provider input. The HTML provider uses a safe renderer that escapes raw HTML, drops unsafe link/image targets, pre-collects safe relative Markdown image tokens, and embeds resolved workspace images as data URLs. Missing or unavailable resources fall back to the existing safe relative image path instead of blocking export. Workbench registers an `Export HTML` command and toolbar action but does not render Markdown, resolve image paths, or write exported files directly. Browser builds use a download fallback when the native export bridge is unavailable.
