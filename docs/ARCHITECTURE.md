...
- `IThemeService`: registered theme contributions and theme token validation
...
`IExtensionService` is the current manifest and runtime API boundary. It accepts extension manifests with command, menu, keybinding, theme, and activation-event contributions, validates required ids and primitive fields, parses menu `when` strings into structured context expressions, registers contributions through the existing command/menu/keybinding/theme services, and returns one disposable that unregisters the whole extension. Registration failures roll back partial contributions. Manifest commands register command metadata and derive `onCommand:<command>` activation events. Manifest themes register through `IThemeService`, which validates Typora Plus CSS token names and rejects unsafe token values before exposing theme metadata. `activateByEvent()` indexes matching extensions, tracks inactive/activating/activated/failed states, and delegates activation to an injected handler with an `ExtensionContext`. That context exposes a constrained command API for registering runtime command handlers, executing commands, and reading command metadata; a constrained context-key API for setting and reading extension-owned keys under the extension id namespace; a constrained export API for registering provider-backed export formats through `IExportService`; and a subscription store owned by the extension record. Runtime command registrations, extension-owned context keys, and extension export providers are cleared when activation fails or when the extension is unregistered. Workbench wires the command service activation handler to `activateByEvent("onCommand:<command>")`, so command palette, keybinding, and menu command execution can activate a matching extension before command execution is retried. Workbench default menus and shortcuts are supplied through a built-in extension manifest, while Workbench command handlers remain registered by `Application.tsx`; a future out-of-process host can use the same context boundary without exposing internal services, DOM access, or unrestricted Node access.
...
- themes
- activation events
- activation context with constrained command registration, command execution, command metadata reads, extension-owned context keys, export-provider registration, and extension-owned subscriptions
