...
- built-in Status inline badge preview for `` `status:done` ``, `` `status:blocked: Waiting on API` ``, and related workflow markers through the Markdown renderer provider path
