...
Successful workspace saves refresh the saved file's index record through `IIndexService.indexFile(file, value?)`. A focused saved-file indexing helper maps the saved model to the current workspace file entry, passes saved content, refreshes the workspace tree once when save-as creates a file that is not listed yet, and synchronizes refreshed workspace state through `IWorkspaceService`. Markdown search, tag, and backlink extraction remain owned by the platform index provider.
