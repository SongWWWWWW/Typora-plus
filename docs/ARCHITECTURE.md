...
Keyboard-driven list surfaces share small Workbench models. Command Palette and Quick Open keep local active-row state, but bounds normalization and Arrow/Home/End movement live in `listNavigationModel.ts`, so new palette-like surfaces can reuse the same behavior without copying key handling into JSX. Quick Open file filtering, ranking, and result limiting live in `workbenchQuickOpenModel.ts`; the maximum result count comes from platform workspace configuration and is exposed through Settings rather than being fixed in the React component. Side view ids, the default side view, toggle behavior, and sidebar titles live in `workbenchSideViewModel.ts`, keeping activity-bar command behavior and sidebar labels aligned with one source of truth.
