...
Titlebar and activitybar actions are resolved through `IMenuService` instead of fixed button lists inside React components. Workbench default actions live in `workbenchContributions.ts` with stable menu ids, command ids, icon ids, order metadata, compact visibility, and toggle context. `Application.tsx` renders those contributions and maps icon ids to React icons locally, keeping platform menu data renderer-agnostic. This mirrors VS Code-style contribution registration: commands, keybindings, and menus are separate contribution points that can later be fed by extensions without rewriting the shell.
