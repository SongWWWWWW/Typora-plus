...
- registered inline Markdown renderer providers can render language-qualified inline code spans such as `` `badge:done` `` through the same Workbench adapter boundary, with editor-owned inline HTML sanitization and source-focused click editing
...
- persisted configuration service for appearance, including custom theme selection, editor, including auto-save delay and renderer preview cache size, and workspace preferences, with platform-owned numeric constraints, stored-value clamping, Electron native storage, and browser fallback
