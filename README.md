...
- platform-level AI service boundary, registered in Workbench, for provider-backed text requests, keeping future OpenAI, Codex, local-model, or workspace-grounded assistants behind registered providers instead of UI-owned integrations
...
- platform-level remote sync service boundary, registered in Workbench, for provider-backed workspace mirroring, workspace resource normalization, stable diff planning with explicit operation targets, execution, and conflict reporting so future Feishu Drive or other cloud integrations stay out of UI code
