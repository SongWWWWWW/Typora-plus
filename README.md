...
- platform-level AI service boundary, registered and observed in Workbench, for provider-backed text requests, observable provider lifecycle changes, provider-availability context keys, and stable default provider selection, keeping future OpenAI, Codex, local-model, or workspace-grounded assistants behind registered providers instead of UI-owned integrations
